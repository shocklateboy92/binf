    ...
    implements GlobalDefines
{
    ...
    public VpnpDpnpJCnoC(MutabilityScore mutabilityScore)
    {
        ...
        G_mutationSpectrum = new MutationSpectrum("Mutation_spectrum.txt");

        try
        {
            G_fostream = new PrintWriter(new FileWriter("gene_nucleotide_mutation_probabilities.txt"));
        }
            ...
            double A_probability, int alignmentModelType, int chainType)
    {
		...
		//array lists for D and J sequences
        ...
        DotState X1a = createDotState("X1a"); // end of V when no exo
        DotState X1b = createDotState("X1b"); // end of V when exo
        DotState X2 = createDotState("X2"); // start of N1
        DotState X3 = createDotState("X3"); // end of N1
        DotState X8 = null;
        DotState X9 = null; 
	    ...
	    DotState X11a_states[] = createDotStateArray("X11a", jSequences.size()); //start of J with no exo - for each germline J
	    DotState X11b_states[] = createDotStateArray("X11b", jSequences.size()); //start of J with exo
   ...
   private double getRelativePprob(double probabilities[], int offset)
    ...
    private int findFamilyIndex(String geneName)
    {
        ...
        if(startIGH == -1)
        {
            throw new Error("Invalid D-Gene sequence name, neither IGH nor DIR:" + geneName);
        } else
        {
